...
contract NFTMarketV2 is Initializable, OwnableUpgradeable {
    ...
    // 离线验证存储NFT上架
    function verifyOfflineSignatureAndList(address seller, address nft, uint256 tokenId, address payToken, uint256 price, uint256 deadline, bytes calldata signature) external {
        // 验证离线签名合法性
        bytes32 offlineHash = _hashTypedData(keccak256(abi.encode(OFFLINE_TYPEHASH, seller, nft, tokenId, payToken, price, deadline)));
        address signer = ECDSA.recover(offlineHash, signature);
        require(seller == signer, "MKT: offline signature error");
        
        // 合法则上架
        _list(seller, nft, tokenId, payToken, price, deadline);
    }
